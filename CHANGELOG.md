...
- **The manager leaked its own internals into the chat**: a malformed action envelope was printed to the user as raw JSON with a stray `</lazy_actions>` tag; action chips truncated a path into a different, plausible-looking directory (`…\projects\scratchpad\uc-smoke` shown as `…\projects`) with no truncation marker
