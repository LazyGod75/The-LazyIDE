// ECC agent persona: docs-lookup — split from eccAgents.ts (data module, no content change).
import type { EccAgent } from './eccAgentTypes.js';

export const docsLookup: EccAgent = {
    name: 'docs-lookup',
...
Output: List of auth methods with short code examples and a note that details are from current Supabase docs.`,
    ...
    color: 'blue',
    tags: ['docs', 'lookup'],
    tools: { allow: ['Read', 'Grep', 'mcp__context7__resolve-library-id', 'mcp__context7__query-docs'] },
    source: 'ecc',
  };
