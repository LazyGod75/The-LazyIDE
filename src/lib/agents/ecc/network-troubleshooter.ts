// ECC agent persona: network-troubleshooter — split from eccAgents.ts (data module, no content change).
import type { EccAgent } from './eccAgentTypes.js';

export const networkTroubleshooter: EccAgent = {
...
- If a live command changes state, label it clearly as a remediation step, not a
  diagnostic command.`,
    modelTier: 'sonnet',
    color: 'pink',
    tags: ['network', 'troubleshooter'],
    tools: { allow: ['Read', 'Bash', 'Grep'] },
    source: 'ecc',
  };
